...
void CMFCTriangleDoc::Set3Point(int n, CPoint point)
{
	if (n == 1)
	{
		m_triangleSegment.m_point1.m_x = point.x;
		m_triangleSegment.m_point1.m_y = point.y;
	}
	else if (n == 2)
	{
		m_triangleSegment.m_point2.m_x = point.x;
		m_triangleSegment.m_point2.m_y = point.y;
	}
	else if (n == 3)
	{
		m_triangleSegment.m_point3.m_x = point.x;
		m_triangleSegment.m_point3.m_y = point.y;
	}
}

void CMFCTriangleDoc::CalcNearest(CPoint point)
{
	int delta_x;
	int delta_y;	
	int d1, d2, d3;

	delta_x = m_triangleSegment.m_point1.m_x - point.x;
	delta_y = m_triangleSegment.m_point1.m_y - point.y;
	d1 = delta_x * delta_x + delta_y * delta_y;

	delta_x = m_triangleSegment.m_point2.m_x - point.x;
	delta_y = m_triangleSegment.m_point2.m_y - point.y;
	d2 = delta_x * delta_x + delta_y * delta_y;

	delta_x = m_triangleSegment.m_point3.m_x - point.x;
	delta_y = m_triangleSegment.m_point3.m_y - point.y;
	d3 = delta_x * delta_x + delta_y * delta_y;
...
void CMFCTriangleDoc::UpdateNearest(CPoint point)
{
	Set3Point(m_nearest, point);
}
